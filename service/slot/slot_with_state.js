...
class SlotWithStateService {
    async getAllSlotsWithStates(filter = {}, sort = { field: 'slot_code', order: 'asc' }) {
        ...
        const slotWithStates = slots
            ...
            .map(slot => {
                ...
                };  // only include slot that is in 'empty' state
            }).filter(slotWithState => slotWithState.slot_state);
    ...
    async getAllEmptySlots(filter = {}, sort = { field: 'slot_code', order: 'asc' }) {
        ...
        const slotWithStates = slots
            ...
            }).filter(slotWithState => slotWithState.slot_state && slotWithState.slot_state.name === 'empty');

        debug(`SlotWithState: filtered? : pass`);
        debug(`SlotWithState: sorting..`);
