...
class CaringDetailDAO {
    async createCaringDetail(preset_id, watering_period, next_due, document) {
    ...
    async getAllCaringDetails() {
        ...
        const query = await db('caring_details')
            ...
            .select('id', 'preset_id', 'watering_period', 'next_due', 'document');
    ...
    async updateCaringDetail(id, preset_id, watering_period, next_due, document) {
        ...
        if (document) {     // with document
            ...
            .where('id', id)
            .update({
                preset_id, watering_period, next_due, document, updated_at: db.fn.now()
            })
            .returning('*');
